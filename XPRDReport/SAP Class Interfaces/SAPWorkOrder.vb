...
Public Class SAPWorkOrder : Implements IDataLookup
    ...
    Public Function ReadRMByWO_V2(KodeUnitSAP As String, TglAwal As Date, TglAkhir As Date) As DataSet
        Dim DB As New SAPDBConnection
        Dim SQL As String

        SQL = "SELECT DISTINCT " +
              " ""NoAddOn""               AS ""ID"", " +
              " ""NoWorkOrder""           AS ""Nomor"" " +
              "FROM XPRD_COSTING_ISSUEFORPRODUCTION " +
              "WHERE  ""Routing"" = '" + KodeUnitSAP + "' AND CAST(""TglTransaksi"" AS DATE) BETWEEN CAST('" + TglAwal.ToString("yyyy-MM-dd") + "' AS DATE) AND CAST('" + TglAkhir.ToString("yyyy-MM-dd") + "' AS DATE) AND ""NoAddOn"" != ''"
    ...
    Public Function ReadWOPlanQty(Unit As String, NomorWO As String) As DataSet
        ...
        SQL = "SELECT * FROM ""HSP_RPT_PLAN_WO_SAP"" " +
