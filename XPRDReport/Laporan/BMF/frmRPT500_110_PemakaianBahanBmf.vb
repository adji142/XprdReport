...
Public Class frmRPT500_110_PemakaianBahanBmf
    ...
    Private Sub GetWO()
        ...
        DS = SAP.ReadRMByWO_V2("MULTIFILAMENT", txtTglAwal.Value.Date, txtTglAkhir.Value.Date)

        Dim DaftarWO As New X_DaftarPemakaianBahan(ActiveSession)
        Dim tmpWO As New TmpWO

        'Hapus Temporary
        DaftarWO.Delete_RMWO()

        Dim Record As Integer = DS.Tables("View").Rows.Count

        For Each DR As DataRow In DS.Tables("View").Rows
            tmpWO.NoTransaksi = DR("ID")
            tmpWO.NomorWO = DR("Nomor")

            DaftarWO.Add_RMWO(tmpWO)
        Next
    End Sub
    ...
    Private Sub btRefresh_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btRefresh.Click
        ...
        Select Case cboTipeLaporan.SelectedIndex + 1
            Case 1
                RPTObject.Load(System.AppDomain.CurrentDomain.BaseDirectory() + "\Reports\System\rpt500121_Bmf.RPT")
            Case 2
