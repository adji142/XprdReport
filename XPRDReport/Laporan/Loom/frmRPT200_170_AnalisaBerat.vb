...
Public Class frmRPT200_170_AnalisaBerat
    ...
    Private Sub FillCombo()
        ...
        Dim DS As DataSet
        DS = New DataSet
        DS = DaftarUnitProduksi.Read("%", UnitProduksi.enumKelompokProduksi.Loom)
    ...
    Private Sub AmbilDataStockSap()
        ...
        DS = SAP.ReadItemRoll(New DaftarUnitProduksi(ActiveSession).Find(cboUnitProduksi.ComboBox.SelectedValue).KodeUnitSAP)

        Dim DaftarTmpItemRoll As New DaftarTmpItemRoll(ActiveSession)
        Dim TmpItemRoll As New TmpItemRoll
        ...
        For Each DR As DataRow In DS.Tables("View").Rows
            TmpItemRoll.KodeItem = DR("KodeItem")
            TmpItemRoll.NamaItem = DR("NamaItem")
            TmpItemRoll.BeratStandart = DR("BeratStandart")

            DaftarTmpItemRoll.Add(TmpItemRoll)
    ...
    Private Sub btRefresh_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btRefresh.Click
        ...
        'Report
        RPTObject.Load(System.AppDomain.CurrentDomain.BaseDirectory() + "\Reports\System\rpt200170_AnalisaBeratRoll.RPT")
