...
Public Class frmRPT7000_Monitoring_WO_ListPPIC
    ...
    Private Sub FillCombo()
        ...
        cboKodeUnit.ComboBox.ValueMember = "Unit SAP"
    ...
    Private Sub btRefresh_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btRefresh.Click
        ...
        Select Case cboTipeLaporan.SelectedIndex + 1
            Case 1
                RPTObject.Load(System.AppDomain.CurrentDomain.BaseDirectory() + "\Reports\System\Rpt70001_LaporanMonitoringWOList.RPT")
            Case 2
                RPTObject.Load(System.AppDomain.CurrentDomain.BaseDirectory() + "\Reports\System\Rpt70002_LaporanMonitoringWOList.RPT")
        End Select
        ...
        'Parameter
        RPTObject.ParameterFields("START_DATE").CurrentValues.AddValue(txtTglAwal.Value.Date)
        RPTObject.ParameterFields("END_DATE").CurrentValues.AddValue(txtTglAwal.Value.Date)
        RPTObject.ParameterFields("ROUTING").CurrentValues.AddValue(cboKodeUnit.ComboBox.SelectedValue)

        'Informasi
        RPTObject.DataDefinition.FormulaFields("UserID").Text = "'" + ActiveSession.KodeUser + "'"
        RPTObject.DataDefinition.FormulaFields("NamaPerusahaan").Text = "'" + UCase(ActiveSession.NamaPerusahaan) + "'"
        If txtTglAwal.Value.ToString("dd/MM/yy") = txtTglAwal.Value.ToString("dd/MM/yy") Then
            RPTObject.DataDefinition.FormulaFields("Periode").Text = "'TANGGAL :  " + txtTglAwal.Value.ToString("dd-MM-yyyy") + "'"
        Else
            RPTObject.DataDefinition.FormulaFields("Periode").Text = "'" + txtTglAwal.Value.ToString("dd-MM-yy") + " S/D " + txtTglAwal.Value.ToString("dd-MM-yy") + "'"
        End If
